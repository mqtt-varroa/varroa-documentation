...
\begin{lstlisting}[caption={example XML definition of the Scenario}, captionpos=b, label={lst:scenario}, language=XML]
<scenario>
	<broker id="b1">
...
\section{Broker}
\begin{lstlisting}[caption={example XML definition of the Broker}, captionpos=b, label={lst:broker}, language=XML]
<scenario>
	<broker id="b1">
...
\begin{lstlisting}[caption={example XML definition of Client Groups}, captionpos=b, label={lst:clientGroups}, language=XML]
<scenario>
	<clientGroups>
		<clientGroup id="cg1">
...
\begin{lstlisting}[caption={example XML definition of MQTT properties}, captionpos=b, label={lst:mqttProperties}, language=XML]
<mqtt>
	<version>5</version>
<\mqtt>
\end{lstlisting}
...
\begin{lstlisting}[caption={example XML definition of Topic Groups}, captionpos=b, label={lst:topicGroups}, language=XML]
<scenario>
	<topicGroups>
		<topicGroup id="tg1">
...
\begin{lstlisting}[caption={example XML definition of subscriptions}, captionpos=b, label={lst:subscriptions}, language=XML]
<scenario>
	<subscriptions>
...
\begin{lstlisting}[caption={example XML definition of waitPatterns}, captionpos=b, label={lst:waitPatterns}, language=XML]
<scenario>
	<waitPatterns>
...
\end{lstlisting}
A Wait Pattern enables the user to module waiting behaviours where MQTT clients waits for receiving messages in a Subscription (see \ref{sec:subscriptions}) before executing other Commands.
...
\section{Stages}
\begin{lstlisting}[caption={example XML definition of Stages}, captionpos=b, label={lst:stages}, language=XML]
<scenario>
	<stages>
...
These Stages are executed sequentially in the order in which they are specified in the XML document.
...
\end{itemize}
A Lifecycle contains an amount of Commands (see \ref{sec:commands}) that must be executed by a Client Group.
...
A Command is to be executed by a Client Group, it must be contained in a Lifecycle.
\subsection{connect}
\begin{lstlisting}[caption={example XML definition of a connect command}, captionpos=b, label={lst:connect}, language=XML]
<connect broker="b1" expectedDuration="10s"/>
\end{lstlisting}
The connect command orders the Client Group to connect to a broker.
...
\begin{lstlisting}[caption={example XML definition of a disconnect command}, captionpos=b, label={lst:disconnect}, language=XML]
<disconnect/>
\end{lstlisting}
...
\subsection{publish}
\begin{lstlisting}[caption={example XML definition of a publish commmand}, captionpos=b, label={lst:publish}, language=XML]
...
\end{lstlisting}
A publish Command orders the Client Group to publish messages.
It has the following parameters:
\begin{itemize}
	...
	\item \textbf{payloadGenerator (optional):} the generator for the payload that is sent within the message. The following options exist:
		\begin{itemize}
			...
			\item \textbf{mustache:} generates payloads bases on a mustache.js template given in the message parameter.
...
\begin{lstlisting}[caption={example XML definition of a subscibe command}, captionpos=b, label={lst:subscirbe}, language=XML]
<subscribe subscription="sub1" qos="1"/>
\end{lstlisting}
A subscribe Command orders the Client Group to subscribe.
It has the following parameters:
\begin{itemize}
...
\begin{lstlisting}[caption={example XML definition of a unsubscibe command}, captionpos=b, label={lst:unsubscirbe}, language=XML]
<unsubscribe subscription="sub1"/>
\end{lstlisting}
A unsubscribe Command orders the Client Group to unsubscribe.
It has the following parameters:
\begin{itemize}
...
\subsection{wait}
\begin{lstlisting}[caption={example XML definition of a wait command}, captionpos=b, label={lst:wait}, language=XML]
<wait waitPattern="wp1" duration="10s"/>
\end{lstlisting}
A wait Command orders a Client Group to wait before executing the following Commands.
...
\subsection{for}
\begin{lstlisting}[caption={example XML definition of a for command}, captionpos=b, label={lst:for}, language=XML]
<for times="2">
	<connect broker="b1"/>
	<disconnect/>
</for>
\end{lstlisting}
A for Command orders the Client Group to execute the inner Commands multiple times.
It has the following parameters:
\begin{itemize}
...
\subsection{rampUp}
\begin{lstlisting}[caption={example XML definition of a rampUp command}, captionpos=b, label={lst:rampUp}, language=XML]
<rampUp duration="10s"/>	
\end{lstlisting}
The Client Group executes the following Commands of the lifecycle in staggered order.
\begin{itemize}
