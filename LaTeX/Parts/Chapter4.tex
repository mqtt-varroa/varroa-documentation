...
<publish topicGroup="tg1" count="10" payloadGenerator="{{clientId}} {{topic}}"/>
<publish topicGroup="tg1" count="10" message="message" payloadGenerator="payload[0-9]{10}"/>
\end{lstlisting}
...
\begin{itemize}
	...
	\item \textbf{payloadGenerator (optional):} the generator for the payload that is sent within the message. The following options exist:
		\begin{itemize}
			...
			\item \textbf{static (default):} uses the payload parameter as payload.
			\item \textbf{mustace:} generates payloads bases on mustache.js templates.
			\item \textbf{regex:} generates payloads based on a regex pattern.
		\end{itemize}
